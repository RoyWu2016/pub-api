...
public class InspectionImpl implements Inspection {
	...
	public ResponseEntity<ApiCallResult> createDraft(@ApiParam(required = true) @PathVariable("userId") String userId,
			@ApiParam(value = "must be one of psi, clc, pm, dupro, ipc", required = true) @RequestParam(value = "serviceType") String serviceType) {
	...
	public ResponseEntity<ApiCallResult> createDraftFromPreviousOrder(
			...
			@ApiParam(value = "previous order id", required = true) @PathVariable("orderId") String orderId,
			@ApiParam(value = "must be one of psi, clc, pm, dupro, ipc", required = true) @RequestParam("serviceType") String serviceType) {
	...
	public ResponseEntity<ApiCallResult> getDraft(@ApiParam(required = true) @PathVariable("userId") String userId,
	...
	public ResponseEntity<ApiCallResult> saveDraft(@ApiParam(required = true) @PathVariable("userId") String userId,
			@ApiParam(required = true) @PathVariable("draftId") String draftId,
			@ApiParam(required = true) @RequestBody ApiInspectionBookingBean draft) {
		logger.info("invoke: " + "/user/" + userId + "/inspection-draft/" + draftId);
	...
	public ResponseEntity<ApiCallResult> createOrderByDraft(
			@ApiParam(required = true) @PathVariable("userId") String userId,
			@ApiParam(required = true) @RequestParam("draftId") String draftId) {
	...
	public ResponseEntity<ApiCallResult> editOrder(@ApiParam(required = true) @PathVariable("userId") String userId,
	...
	@RequestMapping(value = "/user/{userId}/inspection-draft/{draftId}/sampling-level/{samplingLevel}/price", method = RequestMethod.GET)
	public ResponseEntity<ApiCallResult> calculatePricing(
			...
			@ApiParam(value = "must be one of 100%, III, II, I, S4, S3, S2, S1", required = false) @RequestParam(value = "measurementSamplingSize", defaultValue = "II") String measurementSamplingSize) {
		ApiCallResult result = inspectionService.calculatePricing(userId, draftId, samplingLevel,
	...
	public ResponseEntity<ApiCallResult> searchOrders(@ApiParam(required = true) @PathVariable("userId") String userId,
		...
		if ("".equals(orderStatus)) {
			...
			for (int i = 0; i < str.length; i++) {
				if (com.ai.api.bean.consts.ConstMap.STATUS.contains(str[i])) {
					status.add(str[i]);
				}
			}
			if (status.size() <= 0) {
	...
	public ResponseEntity<ApiCallResult> deleteDrafts(@ApiParam(required = true) @PathVariable("userId") String userId,
			@ApiParam(value = "must be comma delimited values", required = true) @RequestParam("draftIds") String draftIds) {
		ApiCallResult result = new ApiCallResult();
		try {
